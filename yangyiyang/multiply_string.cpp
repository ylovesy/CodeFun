 class BigInt {
     ...
     // 两数第i, j位相乘，累加到结果的第i+j位 z[i + j] += y.elems[i] * x.elems[j];
     // if (z[i + j] >= BIGINT_RADIX) { // 看是否要进位 z[i+j+1]+=z[i+j]/BIGINT_RADIX;// 进位
     //     z[i + j] %= BIGINT_RADIX;
     //                     }
     //                     } }
     ...
     //                                 */
     //                                 const static int BIGINT_RADIX = 1000000000; const static int RADIX_LEN = 9;
     //                                 /** 万进制整数. */
